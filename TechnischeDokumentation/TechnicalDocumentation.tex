...
\begin{flushleft}
Wenn z.B. im Beispielprogramm Thread1 (T1) vor Thread3 (T3) in den Lock(a) Bereich kommt, schreibt er beim Verlassen des gesicherten Bereichs einen Eintrag in die Lock-History. Wenn nun T3 den Lock(a) beziehen möchte, prüft der Algorithmus zuerst, ob ein Eintrag in der Lock-History vorhanden ist. Ist die der Fall, wird die eigene Vector Cock (a3, b3, c3) mit der Vector Clock des History Eintrags (a1, b1, c1) synchronisiert.\\
Die neue Vector Clock von T3 setzt sich folgendermassen zusammen:
\begin{center}(MAX(a1, a3), MAX(b1, b3), MAX(c1, c3) + 1)\end{center}
\textit{Synchronisation:}
\begin{itemize}
